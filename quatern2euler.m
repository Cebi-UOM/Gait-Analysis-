...
%   float pole = M_PI/2.0 - 0.05f;                % fix roll near poles with this tolerance
% 
%   theta(1,:) = asin(2.0 * (q(:,1) * q(:,3) - q(:,2) * q(:,4)));
% 
%   if (theta(1,:)< pole) && (theta(1,:) > -pole))
% 	  phi(1,:) = atan2(2.0 * (q(:,3) * q(:,4) + q(:,1) * q(:,2)),...
%                     1.0 - 2.0 * (q(:,2) * q(:,2) + q(:,3) * q(:,3)));
%   end
% 	
%      psi(1,:) = atan2(2.0 * (q(:,2) * q(:,3) + q(:,1) * q(:,4)),...
